...
myMaximum4 ls = foldr1 max ls

myMaximum5 [x] = x
myMaximum5 (x : xs) = max x (myMaximum5 xs)
...
palindrom2 ls = if head ls == last ls then palindrom2 (init $ tail ls) else False
...
pDec2 x p = [1 + (p * hatvany) | (1, hatvany) <- zip (szamjegyek x p) [0 ..]]
    where 
        szamjegyek x p
            | x < 10 = [x]
